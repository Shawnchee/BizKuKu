...
const AVATAR_SUPPORTED_REGIONS = [
  ...
  'westeurope',
  'westus2',
  'southeastasia', // Keep Southeast Asia but will validate
  'australiaeast',
  'westus3'
]
...
✅ AZURE RESOURCE CHECK:
   ...
   Avatar ICE servers use TTS domain, not general API domain.
        ...
        speechConfig = speechSDK.SpeechConfig.fromSubscription(SPEECH_KEY, effectiveRegion)
        
        speechConfig.speechRecognitionLanguage = language === 'ms' ? 'ms-MY' : 'en-US'
        speechConfig.speechSynthesisVoiceName = language === 'ms' 
          ? 'ms-MY-YasminNeural' 
...
🐛 DEBUG INFO:`, {
  ...
  const initializeSpeechRecognition = useCallback(() => {
    ...
    try {
      ...
      const sttLocales = language === 'ms' ? ['ms-MY', 'en-US'] : ['en-US', 'ms-MY']
  ...
  }, [handleError, onSpeechRecognized, onSpeechEnd, language])
  ...
  const speakNext = useCallback(async (text: string, endingSilenceMs: number = 0) => {
    ...
    try {
      ...
      const currentVoice = currentAvatarConfig?.voice || 
        (language === 'ms' ? 'ms-MY-YasminNeural' : 'en-US-AvaMultilingualNeural')

      // Create SSML with proper formatting
      let ssml = `<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts' xml:lang='${language === 'ms' ? 'ms-MY' : 'en-US'}'><voice name='${currentVoice}'><mstts:leadingsilence-exact value='0'/>${htmlEncode(text)}</voice></speak>`
      
      if (endingSilenceMs > 0) {
        ssml = `<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts' xml:lang='${language === 'ms' ? 'ms-MY' : 'en-US'}'><voice name='${currentVoice}'><mstts:leadingsilence-exact value='0'/>${htmlEncode(text)}<break time='${endingSilenceMs}ms' /></voice></speak>`
      }
  ...
  }, [avatarSynthesizerRef, currentAvatarConfig, language, htmlEncode, handleError])
      ...
      <div className={`flex flex-col items-center justify-center p-6 ${className}`}>
        ...
        <div className="text-center">
          <h3 className="text-lg font-semibold text-red-600 mb-2">
            {language === 'ms' ? 'Ralat Avatar' : 'Avatar Error'}
          </h3>
      ...
      <div className="relative inline-block">
          ...
          {!isLoading && !avatarReady && (
            <div className="w-52 h-52 bg-white rounded-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">
                  {language === 'ms' ? 'Menunggu video avatar...' : 'Waiting for avatar video...'}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="absolute -top-6 -right-12 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-2xl px-6 py-3 shadow-xl transform hover:scale-105 transition-transform duration-300">
          <div className="text-sm font-medium">
            {isListening 
              ? (continuousConversation 
                  ? (language === 'ms' ? 'Mendengar terus...' : 'Always listening...')
                  : (language === 'ms' ? 'Mendengar...' : 'Listening...'))
              : isSpeaking
              ? (language === 'ms' ? 'Bercakap...' : 'Speaking...')
              : isReconnecting
              ? (language === 'ms' ? 'Menyambung semula...' : 'Reconnecting...')
              : (language === 'ms' ? 'Hai! Macam mana saya boleh bantu?' : 'Hi! How can I help?')
            }
          </div>
          {continuousConversation && (
            <div className="text-xs opacity-75 mt-1">
              {language === 'ms' ? 'Mod berterusan aktif' : 'Continuous mode active'}
            </div>
          )}
      ...
      {avatarReady && (
        <div className="flex gap-3">
          ...
          <button
            ...
            title={language === 'ms' ? 'Tutup sesi' : 'Close session'}
          >
